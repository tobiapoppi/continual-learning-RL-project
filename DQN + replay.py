...
import copy
import random
from avalanche_rl.training.strategies.rl_base_strategy import Timestep, RLBaseStrategy
from avalanche_rl.training.strategies.buffers import ReplayMemory, Rollout
...
from avalanche_rl.models.dqn import DQNModel, ConvDeepQN
...
b_plug = BasePlugin()
model = ConvDeepQN(4, (84,84), action_space, batch_norm=True)
...
strategy = DQNStrategy(
    model,
    optimizer,
    per_experience_steps=[Timestep(int(1e2)), Timestep(int(1e2))],  # Imposta il numero di passi per ogni esperienza
    criterion=torch.nn.SmoothL1Loss(),
    rollouts_per_step=-1,  # Imposta il numero di rollouts per ogni step
    max_steps_per_rollout=8,
    replay_memory_size=10000,
    replay_memory_init_size=4000,
    updates_per_step=1,
    batch_size=32,
    initial_epsilon=1.0,
    final_epsilon=0.05,
    ...
    polyak_update_tau=1.0,
    device='cuda' if torch.cuda.is_available() else 'cpu',
    plugins=[],
    reset_replay_on_new_experience=True,
    initial_replay_memory=memory, 
